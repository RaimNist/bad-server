...
import { publicDir, resolveInside } from '../utils/safePublicPath'
...
cardsSchema.pre('findOneAndUpdate', async function deleteOldImage() {
    ...
    if (updateImage && docToUpdate) {
        unlink(
            resolveInside(publicDir(), docToUpdate.image.fileName),
            (err) => console.log(err)
        )
    }
})
...
cardsSchema.post('findOneAndDelete', async (doc: IProduct) => {
    unlink(resolveInside(publicDir(), doc.image.fileName), (err) =>
        console.log(err)
    )
})
